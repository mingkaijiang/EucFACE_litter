...
for (i in 2013:2018) {
    wood_production_flux$rain[wood_production_flux$year==i] <- rain$Rain_mm_Tot.sum[rain$year==i]
}
...
p1 <- ggplot() +
    ...
    geom_errorbar(wood_diff, mapping=aes(x=rain.mean, ymin=delta.mean-delta.sd,
...
delta.wood.sub <- subset(wood_production_flux, year %in%c(2017, 2018))
